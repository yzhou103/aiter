...
#ifndef OPUS_GEMM_BHSD_KARGS_GFX950_DEFINED
#define OPUS_GEMM_BHSD_KARGS_GFX950_DEFINED
// Kernel arguments for the BHSD-fused batch GEMM (split-barrier pipeline).
//
...
// tile_k load reads a contiguous segment within one head -- no transpose
// needed.  B_K must divide head_dim (always true: B_K=64|128, head_dim=512).
//
// Compatibility: set stride_a_head = stride_a_seq to degenerate to standard
// row-major K-continuous layout (BSHD fallback).
struct opus_gemm_bhsd_kargs_gfx950 {
    const void* __restrict__ ptr_a;
    const void* __restrict__ ptr_b;
    void* __restrict__ ptr_c;
    int m;                    // seqlen
    int n;                    // output rank (e.g. o_lora_rank)
    int k;                    // heads_per_group * head_dim
    int batch;                // outer_batch * n_groups
    int stride_a_seq;         // row stride within a head (= head_dim)
    int stride_a_head;        // distance between consecutive heads (= seqlen * head_dim)
    int head_dim;             // power of 2 (e.g. 512)
    int heads_per_group;      // (e.g. 8)
    int stride_b;             // B[N,K] row stride (= K)
    int stride_c;             // C row stride (= N)
    int stride_a_batch;       // A per-batch stride (= heads_per_group * stride_a_head)
    int stride_b_batch;       // B per-batch stride (= N * K)
    int stride_c_batch;       // C per-batch stride (= M * N)
};
#endif

#ifndef OPUS_GEMM_BHSD_SPLITK_KARGS_GFX950_DEFINED
#define OPUS_GEMM_BHSD_SPLITK_KARGS_GFX950_DEFINED
// Kernel arguments for the BHSD-fused batch GEMM (flatmm split-K pipeline).
// Same BHSD address remapping as above, plus split-K workspace fields.
struct opus_gemm_bhsd_splitk_kargs_gfx950 {
    const void* __restrict__ ptr_a;
    const void* __restrict__ ptr_b;
    const opus_splitk_ws_handle* __restrict__ ws_handle;
    void*       __restrict__ ptr_c;
    const void* __restrict__ ptr_bias;      // always nullptr for BHSD
    int m, n, k, batch;
    int split_k;
    int stride_a_seq;         // head_dim
    int stride_a_head;        // seqlen * head_dim
    int head_dim;             // power of 2
    int heads_per_group;
    int stride_b;
    int stride_ws;            // = padded_N
    int stride_c;             // = N
    ...
    int stride_ws_batch;      // = padded_M * padded_N
    int stride_c_batch;       // = M * N
    int stride_bias_batch;    // always 0
};
#endif
